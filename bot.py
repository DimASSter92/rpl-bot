...
def get_main_keyboard():
    """Главное меню с кнопками (всегда внизу экрана)"""
    keyboard = ReplyKeyboardMarkup(
        resize_keyboard=True,  # Автоматическая подгонка размера
    ...
    # Второй ряд
    keyboard.add(
        KeyboardButton("📋 Мои прогнозы"),
        KeyboardButton("❓ Помощь")
    )
    
    # Третий ряд
    keyboard.add(
        KeyboardButton("🏅 Чемпион"),
        KeyboardButton("⬇️ Вылет")
    )
...
async def list_matches_command(message: Message):
    ...
    if not matches:
        await message.answer(
            "📭 Нет активных матчей. Подождите, данные загружаются автоматически.",
            reply_markup=get_main_keyboard()
        )
        return
    text = "📅 *Матчи:*\n\n"
...
async def make_prediction(message: Message):
    ...
    if not match:
        await message.answer(
            "❌ Нет матчей для прогноза. Подождите, данные загружаются.",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    session.commit()
    await message.answer(
        f"✅ Прогноз: {match.team1} {s1}:{s2} {match.team2}",
        reply_markup=get_main_keyboard()
    )

@dp.message(Command("mypredictions"))
async def my_predictions_command(message: Message):
    user = get_or_create_user(message)
...
async def show_rating_command(message: Message):
    user = get_or_create_user(message)
...
async def set_champion_predict(message: Message):
    ...
    session.commit()
    await message.answer(
        f"✅ Прогноз на чемпиона: *{user.champion}* (+10 очков)",
        reply_markup=get_main_keyboard(),
        parse_mode="Markdown"
    )
...
async def set_relegation_predict(message: Message):
    ...
    if len(teams) != 2:
        await message.answer(
            "❌ Укажите ровно 2 команды через запятую.",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    session.commit()
    await message.answer(
        f"✅ Прогноз на вылет: *{teams[0]}, {teams[1]}* (+5 за каждую)",
        reply_markup=get_main_keyboard(),
        parse_mode="Markdown"
    )
...
async def add_match(message: Message):
    ...
    try:
        ...
        session.commit()
        await message.answer(
            f"✅ Матч добавлен: {team1} ⚔️ {team2} ({parts[3]})",
            reply_markup=get_main_keyboard()
        )
    except Exception as e:
...
async def set_result(message: Message):
    ...
    if len(parts) != 4:
        await message.answer(
            "❌ Формат: `/setresult ID_матча голы1 голы2`",
            reply_markup=get_main_keyboard()
        )
        return
    try:
        ...
        if not match:
            await message.answer("❌ Матч не найден.", reply_markup=get_main_keyboard())
            return
        match.score1 = s1
        match.score2 = s2
        match.finished = True
...
async def set_champion_result(message: Message):
    ...
    session.commit()
    await message.answer(
        f"✅ Чемпион: *{champion}* (+10 очков угадавшим)",
        reply_markup=get_main_keyboard(),
        parse_mode="Markdown"
    )
...
async def set_relegated_result(message: Message):
    ...
    if len(parts) != 2:
        await message.answer(
            "❌ Формат: `/setrelegated Команда1, Команда2`",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    session.commit()
    await message.answer(
        f"✅ Вылетевшие: *{relegated[0]}, {relegated[1]}* (+5 за каждую)",
        reply_markup=get_main_keyboard(),
        parse_mode="Markdown"
    )
...
async def show_help(message: Message):
    await message.answer(
        ...
        "• Каждая угаданная команда на вылет — +5 очков\n\n"
        "🤖 Матчи и результаты подгружаются автоматически!\n"
        "📝 Чтобы сделать прогноз, используйте команду `/predict X Y`",
        reply_markup=get_main_keyboard(),
        parse_mode="Markdown"
    )

@dp.message(lambda message: message.text == "🏅 Чемпион")
async def show_champion_info(message: Message):
    user = get_or_create_user(message)
...
@dp.message(lambda message: message.text == "⬇️ Вылет")
async def show_relegation_info(message: Message):
    user = get_or_create_user(message)
